...
let JwtStrategy = JwtStrategy_1 = class JwtStrategy extends (0, passport_1.PassportStrategy)(passport_jwt_1.Strategy) {
    constructor(authService) {
        ...
        this.logger = new BackendLogger_1.BackendLogger(JwtStrategy_1.name);
    }
    async validate(payload) {
        const user = await this.authService.validateUser(payload);
        if (!user) {
            this.logger.debug(`Invalid/expired payload: ${JSON.stringify(payload)}`);
            throw new common_1.UnauthorizedException(error_message_1.LOGIN_ERR);
        }
        session_middleware_1.SessionMiddleware.set(constants_1.SESSION_USER, user);
        return user;
    }
};
